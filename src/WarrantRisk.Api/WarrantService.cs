...
{
    public Task<List<Warrant>> SearchAsync(string? keyword)
    {
        return db.Warrants
            .AsNoTracking()
            .Where(x => string.IsNullOrWhiteSpace(keyword)
                        || x.WarrantId.Contains(keyword))
            .OrderBy(x => x.WarrantId)
            .ToListAsync();
    }

    public async Task<(TrialResult Result, TrialLog Log)> TrialAsync(string id, decimal marketPrice)
    {
        var warrant = await db.Warrants
            .SingleOrDefaultAsync(x => x.WarrantId == id);

        if (warrant is null)
        {
            throw new KeyNotFoundException("找不到權證。");
        }
        ...
        var log = new TrialLog
        {
            ...
            TheoryPrice = result.TheoryPrice,
            HedgeQty = result.HedgeQty,
    ...
    public Task<List<TrialLog>> RecentAsync(string id)
    {
        return db.TrialLogs
            .AsNoTracking()
            .Where(x => x.WarrantId == id)
