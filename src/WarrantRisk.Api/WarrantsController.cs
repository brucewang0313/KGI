...
{
    [HttpGet]
    public async Task<ActionResult<List<Warrant>>> Search([FromQuery] string? keyword)
    {
        return Ok(await service.SearchAsync(keyword));
    }

    [HttpGet("{id}")]
    public async Task<ActionResult<Warrant>> Get(string id)
    {
        var warrant = (await service.SearchAsync(id)).SingleOrDefault(x => x.WarrantId == id);
        return warrant is null ? NotFound() : Ok(warrant);
    }

    [HttpGet("{id}/trials/recent")]
    public async Task<ActionResult<List<TrialLog>>> Recent(string id)
    {
        return Ok(await service.RecentAsync(id));
    }
    ...
    public async Task<IActionResult> Trial(string id, [FromBody] TrialRequest request)
    {
        ...
        try
        {
            var result = await service.TrialAsync(id, request.MarketPrice);
